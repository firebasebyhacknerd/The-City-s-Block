...
export async function generateMetadata({ params }: Props) {
  ...
  ogUrl.searchParams.set("type", listing.propertyType);
...
export default async function PropertyDetailPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    "address": {
      "@type": "PostalAddress",
      "addressLocality": locality?.displayName || "",
              ...
              <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm space-y-3">
                ...
                <MapPanel
                  item={{
                    ...
                    address: listing.address || `${locality.displayName}, ${listing.city}`,
                    title: listing.title,
                  }}
                />
                <div className="pt-1 space-y-1">
                  ...
                  <p className="text-xs leading-5 text-gray-500">{locality.overview}</p>
                  <Link
                    href={`/locality/${encodeURIComponent(listing.city)}/${listing.localitySlug}`}
                    className="inline-block text-xs font-medium text-[#1B4332] hover:underline"
                  >
