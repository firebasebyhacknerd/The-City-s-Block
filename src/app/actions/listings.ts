...
export async function createListingAction(formData: {
  ...
  if (!["owner", "admin"].includes(session.role)) {
    return { ok: false, message: "Only owners can post listings." };
  }
...
export async function getPublicListingsAction(filters?: PublicListingFilters) {
  const qPattern = filters?.q?.trim() ? `%${filters.q.trim()}%` : null;
  const localityPattern = filters?.locality?.trim()
  ...
  const rows = await sql`
    ...
    WHERE l.status = 'active'
      AND (${qPattern}::text IS NULL OR (
        l.title ILIKE ${qPattern}
        OR l.city ILIKE ${qPattern}
        OR l.locality ILIKE ${qPattern}
        OR l.address ILIKE ${qPattern}
      ))
      ...
      AND (${filters?.bhkNum ?? null}::int IS NULL OR l.bhk = ${filters?.bhkNum ?? null})
      AND (${filters?.furnishing ?? null}::text IS NULL OR l.furnishing = ${filters?.furnishing ?? null})
      AND (${filters?.possession ?? null}::text IS NULL OR l.possession = ${filters?.possession ?? null})
      AND (${filters?.minPrice ?? null}::bigint IS NULL OR l.price >= ${filters?.minPrice ?? null})
      AND (${filters?.maxPrice ?? null}::bigint IS NULL OR l.price <= ${filters?.maxPrice ?? null})
      AND (${localityPattern}::text IS NULL OR l.locality ILIKE ${localityPattern} OR l.address ILIKE ${localityPattern})
      AND (${filters?.featured ?? null}::boolean IS NULL OR l.featured = ${filters?.featured ?? null})
    ORDER BY l.featured DESC, l.created_at DESC
  `;

  const sort = filters?.sort ?? "relevance";
  const sorted = [...rows] as DbListing[];
  if (sort === "priceAsc") {
...
export async function getListingByIdAction(idOrSlug: string | number) {
  ...
  if (listing.status !== "active") {
    const session = await getSession();
    if (!session || (session.role !== "admin" && listing.user_id !== session.id)) {
      return null;
    }
  }
  return listing;
