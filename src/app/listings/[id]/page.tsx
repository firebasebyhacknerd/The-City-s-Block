...
import { MapPin, BedDouble, Bath, Maximize2, CheckCircle, Calendar } from "lucide-react";
...
export async function generateMetadata({ params }: Props) {
  ...
  ogUrl.searchParams.set("type", listing.property_type);

  return {
    title: `${listing.title} | The City's Block`,
...
export default async function ListingDetailPage({ params }: Props) {
  ...
  return (
    <main className="container-shell py-10 pb-28 lg:pb-16">
      <script
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_380px]">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm space-y-4 lg:hidden">
            <h3 className="font-semibold text-slate-950">Location</h3>
            <MapPanel
              item={{
                coordinates: { lat: 23.0225, lng: 72.5714 },
                address: listing.address || `${listing.locality ? listing.locality + ", " : ""}${listing.city}`,
                title: listing.title,
              }}
            />
          </div>
        ...
        <ListingContactPanel
          ...
          pricePerSqft={
            listing.area
              ? `₹${Math.round(price / Number(listing.area)).toLocaleString("en-IN")}/sq.ft.`
              : undefined
          }
